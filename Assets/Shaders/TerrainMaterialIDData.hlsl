...
float3 TangentToWorld(float3 normalTS, float3 geomNormalWS)
{
    float4 tangentWS = ConstructTerrainTangent(geomNormalWS, GetObjectToWorldMatrix()._13_23_33);
    float3x3 tangentToWorld = BuildTangentToWorld(tangentWS, geomNormalWS);
...
void GetSurfaceAndBuiltinData(inout FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData RAY_TRACING_OPTIONAL_PARAMETERS)
{
    ...
    TerrainLitShade(input.texCoord0.xy, input.positionRWS, terrainLitSurfaceData);
    ...
    #if defined(_OUTPUT_PAGE_ID_TEXTURE)
    OutputPageID(GetAbsolutePositionWS(input.positionRWS), posInput.positionSS);
    #endif
